...
func (c *dexClient) login(ctx context.Context, login string, password string) (string, error) {
	...
	c.log.V(6).Info("Login URL detected", "url", loginURL.String())
	...
	c.log.V(6).Info("Login successful")
...
func (c *dexClient) fetchLoginURL(ctx context.Context) (*url.URL, error) {
	...
	c.log.V(6).Info("Fetching OIDC login page", "url", loginURL.String())
	...
	c.client.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		c.log.V(6).Info("Rejecting redirect", "location", req.URL.String())
	...
	c.log.V(6).Info("Found Location header", "location", location.String())
...
func (c *dexClient) authenticate(ctx context.Context, loginURL *url.URL, login string, password string) (string, error) {
	...
	c.log.V(6).Info("Sending login request", "url", loginURL.String(), "login", login)
	...
	c.client.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == "" || req.URL.Host == loginURL.Host {
			c.log.V(6).Info("Allowing redirect", "location", req.URL.String())
			return nil
		}

		// otherwise stop
		c.log.V(6).Info("Stopping redirect chain", "location", req.URL.String())
		return http.ErrUseLastResponse
	}
	...
	c.log.V(6).Info("Found Location header")
